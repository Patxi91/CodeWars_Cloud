...
def find(t, v):
    return any([t[0], t[1]] == v[i:i + 2] for i in range(len(v) - 1))
...
def check_hints(colors: list, hints: list) -> list:
    ...
    for hint in list(set(hints)):
        ...
        if hint_s[-1] == "left":
            color2, color1 = color1, hint_s[2]
            c_t.append((color1, color2))
        else:
            color2 = hint_s[2]
            c_t.append((color1, color2))
    c = [comb for comb in list(itertools.permutations(colors, len(colors))) for color1, color2 in c_t if
         find((color1, color2), list(comb))]
